...
export default function CertificationsPage() {
  ...
  const catalogItems = certifications.map(({ id, slug, name, category, level, schedules, officialUrl }) => {
    ...
    return {
      ...
      officialUrl,
      nextSchedule: nextSchedule ? `${nextSchedule.date} ${nextSchedule.label}` : "2026 일정 확인",
    };
  });
    ...
    <>
      <JsonLd
        value={itemListJsonLd(
          getDetailCertifications().map((certification) => ({ name: certification.name, path: certificationPath(certification) })),
        )}
      />
